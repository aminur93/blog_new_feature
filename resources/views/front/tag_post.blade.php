                ...
                <div class="row">
                    <div class="span8">
                        <div class="post-image">
                            <a href="#"><img src="/assets/uploads/original_image/{{$tag_post->image }}" alt="" /></a>
                        </div>
                        <ul class="post-meta">
                            <li class="first"><i class="icon-calendar"></i><span><?= date('d F Y',strtotime($tag_post->post_date))?></span></li>
                            <li><i class="icon-comments"></i><span><a href="#">4 comments</a></span></li>
                            <li class="last"><i class="icon-tags"></i><span><a href="#">{{ $tag_post->tag_name }}</a></span></li>
                            <li class="last"><i class="icon-reorder"></i><span><a href="{{ route('post.category_post',$tag_post->id) }}">{{ $tag_post->cat_name }}</a></span></li>
                        </ul>
                        <div class="clearfix">
                        </div>
                        <p>
                            <?= substr(strip_tags($tag_post->body),0,200)?>
                        </p>
                        <a href="{{ route('single.post',$tag_post->slug) }}" class="btn btn-small btn-theme">Read more</a>
                    </div>
                </div>
            </article>
            <!-- end article 1 -->
